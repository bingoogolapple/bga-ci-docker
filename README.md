...
```shell
docker stop jenkins-master && docker rm jenkins-master && docker volume rm jenkins-master-home && docker rmi jenkins-master

echo "" > jenkins-master/Dockerfile && vim jenkins-master/Dockerfile
...
```shell
docker stop jenkins-slave1 && docker rm jenkins-slave1 && docker rmi jenkins-slave-jnlp

echo "" > jenkins-slave-jnlp/Dockerfile && vim jenkins-slave-jnlp/Dockerfile
...
```shell
docker stop android && docker rmi android

echo "" > android/Dockerfile && vim android/Dockerfile

docker build --file android/Dockerfile -t android .

docker run -it --name android android bash
```